...
namespace Shell.Interpreter
{
    public class State
    {
        ...
        internal List<
            Tuple<
                Context<Shell.Types.IShellData>,
                Context<Shell.Types.Function>
            >
        > contexts;

        internal Dictionary<string, Shell.Types.Namespace> all_namespaces;
        ...
        internal Context<Shell.Types.IShellData> CurrentVariables() => contexts.Last().Item1;
        internal Context<Shell.Types.Function> CurrentFunctions() => contexts.Last().Item2;
        ...
        public void ENTER_CONTEXT(string name) {
            var ctx_name = "{" + name + "}";
            var cont = new Context<Shell.Types.IShellData>(ctx_name);
            var new_functions = new Context<Shell.Types.Function>(ctx_name);

            logger.Verbose($"Entering a new context {cont.name} from {CurrentVariables().name}.");
            
            contexts.Add(new Tuple<Context<Types.IShellData>, Context<Types.Function>>(cont, new_functions));
        }
        ...
        public class NameHandler
        {
            ...
            public bool DefinedAsVariable(string s)
            {
                ...
                if (variables.Exists(s) && variables.Get(s) is Shell.Types.IShellData)
                    return true;
            ...
            public bool DefinedAsIndexable(string s)
            {
                ...
                if (variables.Exists(s) && variables.Get(s) is Shell.Types.IShellIndexable)
                    return true;
            ...
            /// </summary>
            public Shell.Types.IShellNamed DefinitionOf(string s)
            {
                if (Available(s)) {
        ...
        public class VariableHandler
        {
            ...
            public Shell.Types.IShellData Get(string name) => state.CurrentVariables().Get(name);
        ...
        public class FunctionHandler
        {
            ...
            public Shell.Types.Function Get(string name) => state.CurrentFunctions().Get(name);
        ...
        public class NamespaceHandler
        {
            ...
            public Shell.Types.Namespace Get(string name) => state.all_namespaces[name];
